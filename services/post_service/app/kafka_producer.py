...
class KafkaEventProducer:
    ...
    def send_post_view_event(self, post_id: str, user_id: int):
        ...
        logger.info(f"🚀 Preparing to send post view event for post_id={post_id}, user_id={user_id}")
        result = self.send_event('post-views', event_data, key=f"post:{post_id}")
        if result:
            logger.info(f"✅ Post view event sent successfully for post_id={post_id}")
        else:
            logger.error(f"❌ Failed to send post view event for post_id={post_id}")
        return result
    ...
    def send_post_like_event(self, post_id: str, user_id: int, action: str):
        ...
        logger.info(f"🚀 Preparing to send post like event for post_id={post_id}, user_id={user_id}, action={action}")
        result = self.send_event('post-likes', event_data, key=f"post:{post_id}")
        if result:
            logger.info(f"✅ Post like event sent successfully for post_id={post_id}")
        else:
            logger.error(f"❌ Failed to send post like event for post_id={post_id}")
        return result
    ...
    def send_post_comment_event(self, post_id: str, user_id: int, comment_id: int, content: str):
        ...
        logger.info(f"🚀 Preparing to send post comment event for post_id={post_id}, user_id={user_id}")
        result = self.send_event('post-comments', event_data, key=f"post:{post_id}")
        if result:
            logger.info(f"✅ Post comment event sent successfully for post_id={post_id}")
        else:
            logger.error(f"❌ Failed to send post comment event for post_id={post_id}")
        return result
